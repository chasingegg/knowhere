...
TEST_CASE("Test Kmeans With Float Vector", "[float metrics]") {
    ...
    SECTION("Test Kmeans result") {
        ...
        auto [name, gen] = GENERATE_REF(table<std::string, std::function<knowhere::Json()>>(
            {make_tuple(knowhere::ClusteringEnum::CLUSTERING_KMEANS, base_gen)}));
        auto clustering = knowhere::ClusteringFactory::Instance().Create<knowhere::fp32>(name).value();
        auto cfg_json = gen().dump();
        CAPTURE(name, cfg_json);
        knowhere::Json json = knowhere::Json::parse(cfg_json);
        REQUIRE(clustering.Type() == name);
        auto res = clustering.Train(*train_ds, json);
        REQUIRE(res.has_value());

        std::vector<std::vector<int64_t>> ids(num_clusters);
        for (int64_t i = 0; i < nb; ++i) {
            auto centroid_id = reinterpret_cast<const uint32_t*>(res.value()->GetCentroidIdMapping())[i];
            REQUIRE(centroid_id < num_clusters);
            ids[centroid_id].push_back(i);
        }
        auto assign_res = clustering.Assign(*query_ds, json);
        ...
        for (int64_t i = 0; i < nq; ++i) {
            auto centroid_id = reinterpret_cast<const uint32_t*>(assign_res.value()->GetCentroidIdMapping())[i];
