...
KMeans<VecT>::fit(const VecT* vecs, size_t n, size_t max_iter, uint32_t random_state, std::string_view init,
    ...
    cluster_id_mapping_ = std::make_unique<uint32_t[]>(n);
    auto closest_centroid_distance = std::make_unique<float[]>(n);

    for (size_t iter = 1; iter <= max_iter; ++iter) {
        if (algorithm == "lloyd") {
            auto loss = lloyds_iter(vecs, closest_docs, cluster_id_mapping_.get(), closest_centroid_distance.get(), n,
...
KMeans<VecT>::lloyds_iter(const VecT* train_data, std::vector<std::vector<uint32_t>>& closest_docs,
    ...
    computeClosestCentroid(train_data, n_train, closest_centroid, closest_centroid_distance);
    for (size_t i = 0; i < n_train; ++i) {
...
template <typename VecT>
void
KMeans<VecT>::computeClosestCentroid(const VecT* vecs, size_t n, uint32_t* closest_centroid,
    ...
    for (size_t i = 0; i < block_num; ++i) {
        ...
        futures.emplace_back(pool->push([&, start, end]() {
            if (std::is_same_v<VecT, float>) {
                exhaustive_L2sqr_blas(vecs + start * dim_, centroids_.get(), dim_, end - start, n_centroids_,
                                      closest_centroid + start, closest_centroid_distance + start);
            } else {
                elkan_L2(vecs + start * dim_, centroids_.get(), dim_, end - start, n_centroids_,
                         closest_centroid + start, closest_centroid_distance + start);
            }
        }));
    }
...
// there are several vector data files, and we sample some files for kmeans training under memory constraint
// after that, get all vector data's cluster id mapping for future data assignment
template <typename VecT>
expected<DataSetPtr>
ClusteringMajorCompaction(const std::vector<std::string>& file_paths, const std::string& output_path_prefix,
    ...
    uint64_t sample_num = 0, total_num = 0;
    uint64_t npts = 0, dim = 0;
    int sample_files = 0;
    std::vector<size_t> vec_num(file_paths.size());
    for (size_t i = 0; i < file_paths.size(); i++) {
        if (!get_bin_metadata(file_paths[i], npts, dim)) {
            return expected<DataSetPtr>::Err(Status::disk_file_error, "");
        }
        if ((sample_num + npts) * dim * sizeof(VecT) <= MAX_TRAIN_SIZE || sample_num == 0) {
            sample_num += npts;
            sample_files++;
        }
        total_num += npts;
    ...
    LOG_KNOWHERE_INFO_ << "total vector num: " << total_num << " dim: " << dim << " sample num: " << sample_num;
    auto data = std::make_unique<VecT[]>(sample_num * dim);
    uint64_t offset = 0;
    for (int i = 0; i < sample_files; i++) {
        if (!load_bin_file<VecT>(file_paths[i], data, offset)) {
            return expected<DataSetPtr>::Err(Status::disk_file_error, file_paths[i] + " read fail");
        }
    }
    ...
    if (sample_num < total_num) {
        auto cluster_id_mapping_all = std::make_unique<uint32_t[]>(total_num);
        std::memcpy(cluster_id_mapping_all.get(), cluster_id_mapping.get(), sample_num * sizeof(uint32_t));
        auto cluster_dis = std::make_unique<float[]>(total_num - sample_num);

        offset = 0;
        for (size_t i = sample_files; i < file_paths.size(); i++) {
            uint64_t dumb = 0;
            auto tmp_data = std::make_unique<VecT[]>(vec_num[i] * dim);
            if (!load_bin_file<VecT>(file_paths[i], tmp_data, dumb)) {
                return expected<DataSetPtr>::Err(Status::disk_file_error, "");
            }
            kMeans.computeClosestCentroid(tmp_data.get(), vec_num[i],
                                          cluster_id_mapping_all.get() + sample_num + offset,
                                          cluster_dis.get() + offset);
            tmp_data.reset();
            offset += vec_num[i];
        }
        if (!write_output_bin(output_path_prefix + "_cluster_id_mapping", cluster_id_mapping_all.get(), total_num, 1)) {
            return expected<DataSetPtr>::Err(Status::disk_file_error, "create cluster id mapping failed");
        }
    } else {
        if (!write_output_bin(output_path_prefix + "_cluster_id_mapping", cluster_id_mapping.get(), total_num, 1)) {
            return expected<DataSetPtr>::Err(Status::disk_file_error, "create cluster id mapping failed");
        }
    }
...
ClusteringMajorCompaction<float>(const std::vector<std::string>& file_paths, const std::string& output_path_prefix,
                                 int num_clusters, std::shared_ptr<FileManager> file_manager);
